function [datTab_selTrl_uniqueCellRows, datCell_selTrl_uniqueCellRows] = rowConcatenateCellsV3(dataTable_selectedTrials, STIM_ONSET_GUESS, STIM_OFFSET_GUESS)
    ...
    for sessionInd = 1:numSessions %%% session
        ...
        for stimTypeInd = 1:numStimTypes %%% stimulus
            ...
            for trialInd = 1:height(tempTable)
                ...
                if trialInd == 1
                    try tempTable.rast{1} = tempTable.rast{1}(timeInd_start:timeInd_start);
                    catch ME
                        disp(ME.identifier);
                    end
